...
static void print_stats(t_block *clb, int num_clusters, int procRank, int procSize) {
...
/*counting number of flipflops which cannot be absorbed to check the optimality of the packer wrt CLB density*/
	...
	for (itype = 0; itype < num_types; itype++) {
		if (num_clb_types[itype] == 0) {
			vpr_printf(TIO_MESSAGE_INFO, "(output_clustering) \t#%d/%d << %s: # blocks: %d, average # input + clock pins used: %g, average # output pins used: %g\n",
					procRank, procSize, type_descriptors[itype].name, num_clb_types[itype], 0.0, 0.0);
		} else {
			vpr_printf(TIO_MESSAGE_INFO, "(output_clustering) \t#%d/%d << %s: # blocks: %d, average # input + clock pins used: %g, average # output pins used: %g\n",
					procRank, procSize, type_descriptors[itype].name, num_clb_types[itype],
	...
	vpr_printf(TIO_MESSAGE_INFO, "(output_clustering) #%d/%d << Absorbed logical nets %d out of %d nets, %d nets not absorbed.\n",
			procRank, procSize, total_nets_absorbed, num_logical_nets, num_logical_nets - total_nets_absorbed);
	free(nets_absorbed);
	free(num_clb_types);
...
void output_clustering(t_block *clb, int num_clusters, boolean global_clocks,
    ...
    int mpiRank, mpiSize; // MPI variables 
	...
	for (bnum = 0; bnum < num_logical_blocks; bnum++) {
		...
		case VPACK_EMPTY:
			vpr_printf(TIO_MESSAGE_ERROR, "(output_clustering) #%d/%d << in output_netlist: logical_block %d is VPACK_EMPTY.\n", mpiRank, mpiSize,
					bnum);
			exit(1);
			break;

		default:
			vpr_printf(TIO_MESSAGE_ERROR, "(output_clustering) #%d/%d << in output_netlist: Unexpected type %d for logical_block %d.\n", mpiRank, mpiSize, 
	...
	print_stats(clb, num_clusters, mpiRank, mpiSize);
}
