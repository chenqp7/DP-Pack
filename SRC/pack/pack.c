...
void try_pack(INP struct s_packer_opts *packer_opts, INP const t_arch * arch,
	...
	vpr_printf(TIO_MESSAGE_INFO, "(pack) #%d/%d << Begin packing '%s'.\n", mpiRank, mpiSize, packer_opts->blif_file_name);
	...
	vpr_printf(TIO_MESSAGE_INFO, "(pack) #%d/%d << After removing unused inputs...\n", mpiRank, mpiSize);
	vpr_printf(TIO_MESSAGE_INFO, "(pack) \t\t#%d/%d << total blocks: %d, total nets: %d, total inputs: %d, total outputs: %d\n", mpiRank, mpiSize,
			num_logical_blocks, num_logical_nets, num_p_inputs, num_p_outputs);

	vpr_printf(TIO_MESSAGE_INFO, "(pack) #%d/%d << Begin prepacking.\n", mpiRank, mpiSize);
	...
	list_of_pack_molecules = alloc_and_load_pack_molecules(
			...
			&num_pack_molecules);
	vpr_printf(TIO_MESSAGE_INFO, "(pack) #%d/%d << Finish prepacking.\n", mpiRank, mpiSize);

	if(packer_opts->auto_compute_inter_cluster_net_delay) {
		packer_opts->inter_cluster_net_delay = interc_delay;
		vpr_printf(TIO_MESSAGE_INFO, "(pack) #%d/%d << Using inter-cluster delay: %g\n", mpiRank, mpiSize, packer_opts->inter_cluster_net_delay);
	}
	...
	if (packer_opts->skip_clustering == FALSE) {
		...
		vpr_printf(TIO_MESSAGE_ERROR, "(pack) #%d/%d << Skip clustering no longer supported.\n", mpiRank, mpiSize);
		exit(1);
	}
	...
	vpr_printf(TIO_MESSAGE_INFO, "\n");
	vpr_printf(TIO_MESSAGE_INFO, "(pack) #%d/%d << Netlist conversion complete.\n", mpiRank, mpiSize);
	vpr_printf(TIO_MESSAGE_INFO, "\n");
}
