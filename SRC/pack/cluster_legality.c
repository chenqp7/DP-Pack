...
int save_cluster_solution_with_message(int num_clusters_solution, int * clusters_solution) {

	int i, j, net_index;
	struct s_trace *tptr, *prev;
	int inode;
    int iposition = num_clusters_solution;
	for (i = 0; i < max_ext_index; i++) {
		rr_node[i].net_num = OPEN;
		rr_node[i].prev_edge = OPEN;
		rr_node[i].prev_node = OPEN;
	...
	for (i = 0; i < num_nets_in_cluster; i++) {
		...
		tptr = trace_head[net_index];
		if (tptr == NULL) {/* No routing yet. */
            clusters_solution[iposition] = 2147483647;
            iposition++;
		...
		for (;;) {
			...
			rr_node[inode].net_num = net_index;
            iposition += 6;
            clusters_solution[iposition - 6] = inode;
            clusters_solution[iposition - 5] = net_index;
			if (prev != NULL) {
				rr_node[inode].prev_node = prev->index;
				for (j = 0; j < rr_node[prev->index].num_edges; j++) {
					if (rr_node[prev->index].edges[j] == inode) {
            ...
            clusters_solution[iposition - 4] = rr_node[inode].prev_node;
            clusters_solution[iposition - 3] = rr_node[inode].prev_edge;
        	clusters_solution[iposition - 2] = rr_node[i].occ;
        	clusters_solution[iposition - 1] = rr_node[i].capacity;
	...
	for (i = 0; i < max_ext_index; i++) {
        ...
        clusters_solution[iposition - 6] = i;
        clusters_solution[iposition - 5] = rr_node[i].net_num;
        clusters_solution[iposition - 4] = rr_node[i].prev_node;
        clusters_solution[iposition - 3] = rr_node[i].prev_edge;
        clusters_solution[iposition - 2] = rr_node[i].occ;
        clusters_solution[iposition - 1] = rr_node[i].capacity;
	}
    clusters_solution[iposition] = 2147483647;
...
int collect_cluster_solution(int current_position, int num_clusters_solution, int * clusters_solution) {

	int i;
    int current = current_position;
	for (i = current_position; i < num_clusters_solution; i += 6) {
		if (clusters_solution[i] == 2147483647 && i != (num_clusters_solution - 1)) { /* End current cluster info. */
			return i + 1;
        } 
        if (i == (num_clusters_solution - 1)) {
            return i;
        }

		rr_node[clusters_solution[i]].net_num = clusters_solution[i + 1];
		rr_node[clusters_solution[i]].prev_node = clusters_solution[i + 2];
		rr_node[clusters_solution[i]].prev_edge = clusters_solution[i + 3];
		rr_node[clusters_solution[i]].occ = clusters_solution[i + 4];
		rr_node[clusters_solution[i]].capacity = clusters_solution[i + 5];
        current = i;
	}
    return (current + 7);
}
