...
/* MPI informations */
int mpiRank, mpiSize; 
static boolean is_run_global = FALSE;

/* MPI Send molecule max size */
static int max_num_clb = 65536;
static int max_num_molecule = 128;
static int max_molecule_num_blocks = 32;
...
/* MPI By CQP */
static int my_MPI_Get_msg_size(int recv_rank, int recv_tag, MPI_Datatype recv_type);

static int arr2to1(int x, int y);
...
void do_clustering(const t_arch *arch, t_pack_molecule *molecule_head,
    ...
    // MPI By CQP: Add follow  arrays for merge clb. Will send follow 3 variables
    // modify array size to small, default is (65536, 32, 8)
    //max_num_clb = 1000;
    //max_num_molecule = 10;
    //max_molecule_num_blocks = 3; 
    int *num_clb_molecules = (int *)my_calloc(max_num_clb, sizeof(int));
//    float *clb_molecules_base_gain = (float *)calloc(max_num_clb*max_num_molecule, sizeof(float));
//    int *clb_molecules_rootblk_indexs = (int *)calloc(max_num_clb*max_num_molecule, sizeof(int));
    int *clb_molecules_num_blks = (int *)my_calloc(max_num_clb*max_num_molecule, sizeof(int));
    int *clb_molecules_logical_blks_index = (int *)my_calloc(max_num_clb*max_num_molecule*max_molecule_num_blocks, sizeof(int));
//    int **clb_molecules_rootblk_indexs = (int **)calloc(max_num_clb, sizeof(int *));
//    for (int init_index = 0; init_index < max_num_clb; init_index++) {
//        clb_molecules_rootblk_indexs[init_index] = (int *)calloc(max_num_clb, sizeof(int));
//    }
//    int ***clb_molecules_logical_blks_index = (int ***)calloc(max_num_clb, sizeof(int **));
//    for (int init_index = 0; init_index < max_num_molecule; init_index++) {
//        clb_molecules_logical_blks_index[init_index] = (int **)calloc(max_num_molecule, sizeof(int *));
//        for (int init_j = 0; init_j < max_molecule_num_blocks; init_j++) {
//            clb_molecules_logical_blks_index[init_index][init_j] = (int *)calloc(max_molecule_num_blocks, sizeof(int));
//        }
//    }
//
    int num_clbs_solution = 0;
    int *clbs_solution = (int *)my_calloc(max_num_clb*max_num_molecule*max_molecule_num_blocks, sizeof(int));
	num_clb = 0;
	istart = NULL;
    ...
    /****** MPI TODO: use this to separate logical_block by processes number *******/
    /****** But maybe molecules can't use this to separate. TODO: need test ********/
	indexofcrit = 0;
	//indexofcrit = mpiRank; // MPI By CQP
	...
	if (timing_driven) {
		...
		if (cluster_seed_type == VPACK_TIMING) {
            /*********MPI TODO: Need to check if different process choose the same molecule***********/
			istart = get_most_critical_seed_molecule(&indexofcrit);
            /********************* MPI By CQP **********************/
            int istart_root_index = 2147483647; // Max INT means istart == NULL
            if (istart != NULL) { 
                istart_root_index = istart->logical_block_ptrs[istart->root]->index;
            } 
            if (mpiRank == 0) { 
                MPI_Send(&istart_root_index, 1, MPI_INT, mpiRank + 1, 0, MPI_COMM_WORLD); 
            } else { 
                int *recv_start_root_index_array = (int *)calloc(mpiRank, (sizeof(int)));
                MPI_Recv(recv_start_root_index_array, mpiRank, MPI_INT, mpiRank - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                while (istart != NULL) {
                    boolean diff_start = TRUE;
                    for(int recv_index = 0; recv_index < mpiRank; recv_index++) {
                        if(istart_root_index == recv_start_root_index_array[recv_index]) { 
                            diff_start = FALSE;
                        } 
                    }
                    if(diff_start) { 
                        ...
                        if (istart != NULL) { 
                            ...
                            istart_root_index = 2147483647; 
                        } 
                    } 
                } 
                if (mpiRank != mpiSize - 1) { 
                    int *send_start_root_index_array = (int *)calloc(mpiRank + 1, (sizeof(int)));
                    for(int send_index = 0; send_index < mpiRank; send_index++) {
                        send_start_root_index_array[send_index] = recv_start_root_index_array[send_index];
                    }
                    send_start_root_index_array[mpiRank] = istart_root_index;
                    MPI_Send(send_start_root_index_array, mpiRank + 1, MPI_INT, mpiRank + 1, 0, MPI_COMM_WORLD); 
                    free(send_start_root_index_array);
                } 
                free(recv_start_root_index_array);
            } 
            // check which seed type is runned.
			vpr_printf(TIO_MESSAGE_DIRECT, "(cluster) #%d/%d << ####################### Use VPACK_TIMING ######################\n", mpiRank, mpiSize); 
            /***************************************************/
		} else {/*max input seed*/
	...
	while (istart != NULL) {
		...
		for (detailed_routing_stage = (int)E_DETAILED_ROUTE_AT_END_ONLY; !is_cluster_legal && detailed_routing_stage != (int)E_DETAILED_ROUTE_END; detailed_routing_stage++) {
            ...
            /***************MPI TODO: check if cluster->name "cb.%s" is different because of the different seed? **********/
			start_new_cluster(cluster_placement_stats, primitives_list, arch,
                    ...
                    mpiRank, mpiSize);
			vpr_printf(TIO_MESSAGE_INFO, "(cluster) #%d/%d << Complex block %d: %s, type: %s\n", mpiRank, mpiSize, 
			...
			num_clb++;
            // MPI By CQP: record clb molecules
            clb_molecules_num_blks[arr2to1(num_clb - 1, 0)] = istart->num_blocks; 
            num_clb_molecules[num_clb - 1] = 1;
            for (int blkptr_i = 0; blkptr_i < istart->num_blocks; blkptr_i++) {
                clb_molecules_logical_blks_index[arr3to1(num_clb - 1, 0, blkptr_i)] = istart->logical_block_ptrs[blkptr_i]->index;
            }
            //vpr_printf(TIO_MESSAGE_DIRECT, "+++++++++++++++++++++%d; %d; %d#######################\n", num_clb, num_clb_molecules[num_clb - 1], clb_molecules_num_blks[arr2to1(num_clb - 1, 0)]);
            ...
            /***************MPI TODO: First partition logical_block according to mpiRank******************/
            /*************************Then check if next_module is different with different process*********/
			...
			while (next_molecule != NULL && prev_molecule != next_molecule) {
				...
				if (block_pack_status != BLK_PASSED) {
					if (next_molecule != NULL) {
						if (block_pack_status == BLK_FAILED_ROUTE) {
							...
							vpr_printf(TIO_MESSAGE_DIRECT, "\t(cluster) #%d/%d << NO_ROUTE:%s type %s/n", mpiRank, mpiSize, 
	...
	#ifdef DEBUG_FAILED_PACKING_CANDIDATES
							vpr_printf(TIO_MESSAGE_DIRECT, "\t(cluster) #%d/%d << FAILED_CHECK:%s type %s check %d\n", mpiRank, mpiSize, 
									next_molecule->logical_block_ptrs[next_molecule->root]->name, 
									next_molecule->logical_block_ptrs[next_molecule->root]->model->name, 
									block_pack_status);
							fflush(stdout);
	...
	#ifdef DEBUG_FAILED_PACKING_CANDIDATES			
					vpr_printf(TIO_MESSAGE_DIRECT, "\t(cluster) #%d/%d << PASSED:%s type %s\n", mpiRank, mpiSize, 
							next_molecule->logical_block_ptrs[next_molecule->root]->name, 
							next_molecule->logical_block_ptrs[next_molecule->root]->model->name);
					fflush(stdout);
	#else
                ...
                // MPI By CQP: record clb molecules
                if (next_molecule != NULL) { 
                    clb_molecules_num_blks[arr2to1(num_clb - 1, num_clb_molecules[num_clb - 1])] = next_molecule->num_blocks; 
                    for (int blkptr_i = 0; blkptr_i < next_molecule->num_blocks; blkptr_i++) {
                        clb_molecules_logical_blks_index[arr3to1(num_clb - 1, num_clb_molecules[num_clb - 1], blkptr_i)] = next_molecule->logical_block_ptrs[blkptr_i]->index;
                    }
                    num_clb_molecules[num_clb - 1] += 1;
                    //vpr_printf(TIO_MESSAGE_DIRECT, "+++++++++++++++++++++%d: %d; %d#######################\n", num_clb, num_clb_molecules[num_clb - 1], clb_molecules_num_blks[arr2to1(num_clb - 1, num_clb_molecules[num_clb - 1])]);
                } 
			...
			if (detailed_routing_stage == (int)E_DETAILED_ROUTE_AT_END_ONLY) {
				...
				if (is_cluster_legal == TRUE) {
					...
					vpr_printf(TIO_MESSAGE_INFO, "(cluster) #%d/%d << Failed route at end, repack cluster trying detailed routing at each stage.\n", mpiRank, mpiSize);
				}
			} else {
			...
			if (is_cluster_legal == TRUE) {
                ...
                num_clbs_solution = save_cluster_solution_with_message(num_clbs_solution, clbs_solution);
				if (timing_driven) {
    ...
    if (mpiRank != 0) { 
        /* free all alloc variables except for messages*/
        ...
        /* send messages to master process */
        MPI_Send(&num_clb, 1, MPI_INT, 0, 21, MPI_COMM_WORLD);
        MPI_Send(num_clb_molecules, max_num_clb, MPI_INT, 0, 22, MPI_COMM_WORLD);
//        MPI_Send(clb_molecules_base_gain, max_num_clb*max_num_molecule, MPI_INT, 0, 23, MPI_COMM_WORLD);
//        MPI_Send(clb_molecules_rootblk_indexs, max_num_clb*max_num_molecule, MPI_INT, 0, 24, MPI_COMM_WORLD);
        MPI_Send(clb_molecules_num_blks, max_num_clb*max_num_molecule, MPI_INT, 0, 25, MPI_COMM_WORLD);
        MPI_Send(clb_molecules_logical_blks_index, max_num_clb*max_num_molecule*max_molecule_num_blocks, MPI_INT, 0, 26, MPI_COMM_WORLD);
        MPI_Send(&num_clbs_solution, 1, MPI_INT, 0, 27, MPI_COMM_WORLD);
        int * message_nets = (int *)my_calloc(num_clbs_solution, sizeof(int));
        for(int im = 0; im < num_clbs_solution; im++) {
            message_nets[im] = clbs_solution[im];
            //vpr_printf(TIO_MESSAGE_INFO, "%d<<CCCCCCCCCCCCCCCCCCC %d \n", mpiRank, message_nets[im]);
        }
        MPI_Send(message_nets, num_clbs_solution, MPI_INT, 0, 28, MPI_COMM_WORLD);
        //vpr_printf(TIO_MESSAGE_INFO, "XXXXXXXXXXXXXXXX %d XXXXXX %d XXXXXXXX\n", num_clbs_solution, clbs_solution[num_clbs_solution - 1]);
        free(message_nets);
    } else { 
        int recv_clb_num = 0;
        boolean has_packed_m = FALSE;
        boolean is_moved_next = FALSE;
        for (int recv_rank = 1; recv_rank < mpiSize; recv_rank++) {
            MPI_Recv(&recv_clb_num, 1, MPI_INT, recv_rank, 21, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
            MPI_Recv(num_clb_molecules, max_num_clb, MPI_INT, recv_rank, 22, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
//            MPI_Recv(clb_molecules_base_gain, max_num_clb*max_num_molecule, MPI_INT, recv_rank, 23, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
//            MPI_Recv(clb_molecules_rootblk_indexs, max_num_clb*max_num_molecule, MPI_INT, recv_rank, 24, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
            MPI_Recv(clb_molecules_num_blks, max_num_clb*max_num_molecule, MPI_INT, recv_rank, 25, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
            MPI_Recv(clb_molecules_logical_blks_index, max_num_clb*max_num_molecule*max_molecule_num_blocks, MPI_INT, recv_rank, 26, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
            int collect_clbs_num = 0;
            MPI_Recv(&collect_clbs_num, 1, MPI_INT, recv_rank, 27, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
            int * collect_clbs = (int *)my_calloc(collect_clbs_num, sizeof(int));
            MPI_Recv(collect_clbs, collect_clbs_num, MPI_INT, recv_rank, 28, MPI_COMM_WORLD, MPI_STATUS_IGNORE); 
            //vpr_printf(TIO_MESSAGE_INFO, "YYYYYYYYYYYYYYYY %d YYYYYYY %d YYYYYYY\n", collect_clbs_num, collect_clbs[collect_clbs_num - 1]);
            int icollect_position = 0;
            for (int recv_clb_i = 0; recv_clb_i < recv_clb_num; recv_clb_i++) {
		        is_cluster_legal = FALSE;
                has_packed_m = FALSE;
                is_moved_next = FALSE;
		        for (detailed_routing_stage = (int)E_DETAILED_ROUTE_AT_END_ONLY; !is_cluster_legal && detailed_routing_stage != (int)E_DETAILED_ROUTE_END; detailed_routing_stage++) {
                    //istart = (t_pack_molecule *)logical_block[clb_molecules_rootblk_indexs[arr2to1(recv_clb_i, 0)]].packed_molecules->data_vptr;
                    /*boolean confirm_diff_molecule = TRUE;*/
                    boolean blk_no_cluster = FALSE;
                    for (int blks_i = 0; blks_i < clb_molecules_num_blks[arr2to1(recv_clb_i, 0)]; blks_i++) {
                        if (logical_block[clb_molecules_logical_blks_index[arr3to1(recv_clb_i, 0, blks_i)]].clb_index == NO_CLUSTER) {
                            istart = (t_pack_molecule *)logical_block[clb_molecules_logical_blks_index[arr3to1(recv_clb_i, 0, 0)]].packed_molecules->data_vptr;
                            blk_no_cluster = TRUE;
                            break;
/*                            cur_molecule = (t_pack_molecule *)logical_block[clb_molecules_logical_blks_index[arr3to1(recv_clb_i, 0, blks_i)]].packed_molecules->data_vptr;*/
                            /*if (istart->base_gain != cur_molecule->base_gain) { */
                                /*confirm_molecule = FALSE; */
                            /*} */
                        }
                    }
//                    if (logical_block[clb_molecules_logical_blks_index[arr3to1(recv_clb_i, 0, 0)]].clb_index == NO_CLUSTER) { 
//                        istart = (t_pack_molecule *)logical_block[clb_molecules_logical_blks_index[arr3to1(recv_clb_i, 0, 0)]].packed_molecules->data_vptr; 
//                        blk_no_cluster = TRUE;
//                    } 
                    if (blk_no_cluster == FALSE) { 
                        //vpr_printf(TIO_MESSAGE_DIRECT, "\t Seed Molecule from process %d are in some cluster!!! For loop %d\n", recv_rank, recv_clb_i);
                        for(int ip = icollect_position; !is_moved_next && ip < collect_clbs_num; ip++) {
                            if (collect_clbs[ip] == 2147483647) { 
                                icollect_position = ip + 1;
                                is_moved_next = TRUE;
                                break; 
                            } 
                        }
			        ...
			        start_new_cluster(cluster_placement_stats, primitives_list, arch,
			        		...
			        		max_nets_in_pb_type, detailed_routing_stage, 
                            mpiRank, mpiSize);
			        vpr_printf(TIO_MESSAGE_INFO, "(cluster) #%d/%d << collecting clb to Complex block %d: %s, type: %s\n", mpiRank, mpiSize, 
			        		num_clb, clb[num_clb].name, clb[num_clb].type->name);
			        vpr_printf(TIO_MESSAGE_INFO, "\t");
			        fflush(stdout);
			        update_cluster_stats(istart, num_clb, is_clock, global_clocks, alpha,
                    ...
                    num_unrelated_clustering_attempts = 0;
                    for (int clb_moleclues_i = 1; clb_moleclues_i < num_clb_molecules[recv_clb_i]; clb_moleclues_i++) {
                        blk_no_cluster = FALSE;
                        for (int blks_i = 0; blks_i < clb_molecules_num_blks[arr2to1(recv_clb_i, clb_moleclues_i)]; blks_i++) {
                            if (logical_block[clb_molecules_logical_blks_index[arr3to1(recv_clb_i, clb_moleclues_i, blks_i)]].clb_index == NO_CLUSTER) { 
                                next_molecule = (t_pack_molecule *)logical_block[clb_molecules_logical_blks_index[arr3to1(recv_clb_i, clb_moleclues_i, 0)]].packed_molecules->data_vptr;
                                blk_no_cluster = TRUE;
                                break;
                            }
                        } 
                        if (blk_no_cluster == FALSE) { 
                            //vpr_printf(TIO_MESSAGE_DIRECT, "\t One molecule from process %d are in some cluster!!! For loop %d\n", recv_rank, recv_clb_i);
                            has_packed_m = TRUE;
                            continue; 
                        } 
				        block_pack_status = try_pack_molecule(
			        ...
			        if (is_cluster_legal == TRUE) {
                        if (has_packed_m == TRUE) {
                            ...
                            icollect_position = collect_cluster_solution(icollect_position, collect_clbs_num, collect_clbs);
                            is_moved_next = TRUE;
                        }
				        free_pb_stats_recursive(clb[num_clb - 1].pb);
			        } else {
        ...
        // ***************MPI TODO: finished merge, then check missing blks to clustering ***********
        // ******************* This is not the right way, but to fix the ignore blks **************
        is_run_global = TRUE;
        for (i = 0; i < num_logical_blocks; i++) {
            if (logical_block[i].clb_index == NO_CLUSTER) {
                //vpr_printf(TIO_MESSAGE_DIRECT, "has blk%d no cluster\n", i);
                if (timing_driven) { 
		            if (cluster_seed_type == VPACK_TIMING) {
		            	istart = get_most_critical_seed_molecule(&indexofcrit);
	                    t_pack_molecule *molecule;
	                    struct s_linked_vptr *cur;
			            cur = logical_block[i].packed_molecules;
			            istart = NULL;
			            while (cur != NULL) {
			            	molecule = (t_pack_molecule *) cur->data_vptr;
			            	if (molecule->valid) {
			            		if (istart == NULL || (istart->base_gain) < (molecule->base_gain)) {
			            			istart = molecule;
			            		}
			            	}
			            	cur = cur->next;
    ...
    free(num_clb_molecules);
    free(clb_molecules_num_blks);
    free(clb_molecules_logical_blks_index);
...
static t_pack_molecule *get_molecule_by_num_ext_inputs(
	...
	int ilogical_blk, i;
	boolean success;
    boolean is_in_task = FALSE;

	prev_ptr = &unclustered_list_head[ext_inps];
	ptr = unclustered_list_head[ext_inps].next;
	while (ptr != NULL) {
        /******************MPI TODO: By CQP need to complete parallism this func*********************/
        is_in_task = FALSE;
        if (is_run_global) {
            is_in_task = TRUE;
        } 
        if (ptr->moleculeptr->logical_block_ptrs[ptr->moleculeptr->root]->index%mpiSize == mpiRank) {
            is_in_task = TRUE;
        } 
        /**********************************************************************/
		/* TODO: Get better candidate logical block in future, eg. return most timing critical or some other smarter metric */
		if (ptr->moleculeptr->valid && is_in_task) { // MPI By CQP: Add && ...->index%mpiSize == mpiRank
			success = TRUE;
...
static enum e_block_pack_status try_place_logical_block_rec(
	...
	*parent = pb; /* this pb is parent of it's child that called this function */
	...
	if (is_primitive) {
        //vpr_printf(TIO_MESSAGE_DIRECT, "ASSERT!!!: (%d, %d; %ld; %d, %d)", pb->logical_block, OPEN, logical_block[ilogical_block].pb, logical_block[ilogical_block].clb_index, NO_CLUSTER);
		assert(
...
static void start_new_cluster(
	...
	while (!success) {
		count = 0;
		for (i = 0; i < num_types; i++) {
			if (num_used_instances_type[i] < num_instances_type[i]) { // check the instances of types provided by FPGA global grid have unused object
		...
		if (count == num_types - 1) {
			vpr_printf(TIO_MESSAGE_ERROR, "(cluster) #%d/%d << Can not find any logic block that can implement molecule.\n", procRank, procSize);
		...
		if (!success) {
			...
			vpr_printf(TIO_MESSAGE_INFO, "(cluster) #%d/%d << Not enough resources expand FPGA size to x = %d y = %d.\n", procRank, procSize,
					nx, ny);
			if ((nx > MAX_SHORT) || (ny > MAX_SHORT)) {
				vpr_printf(TIO_MESSAGE_ERROR, "(cluster) #%d/%d << Circuit cannot pack into architecture, architecture size (nx = %d, ny = %d) exceeds packer range.\n", procRank, procSize,
						nx, ny);
				exit(1);
			}
...
static t_pack_molecule *get_highest_gain_molecule(
	...
	if (cur_pb->pb_stats->num_feasible_blocks == NOT_VALID) {
		...
		if (cur_pb->pb_stats->num_marked_blocks
			...
			for (iblk = 0; iblk < num_logical_blocks; iblk++) {
                ...
                /*******************************************/
				if (logical_block[iblk].clb_index == NO_CLUSTER) { // MPI By CQP : add iblk%mpiSize == mpiRank && 
					cur = logical_block[iblk].packed_molecules;
					while (cur != NULL) {
						molecule = (t_pack_molecule *) cur->data_vptr;
						if (molecule->valid) {
	...
	if(cur_pb->pb_stats->num_feasible_blocks == 0 && cur_pb->pb_stats->tie_break_high_fanout_net != OPEN) {
		...
		for (i = 0; i <= vpack_net[inet].num_sinks && count < AAPACK_MAX_HIGH_FANOUT_EXPLORE; i++) {
            ...
            /*******************************************/
			if (logical_block[iblk].clb_index == NO_CLUSTER) { // MPI By CQP: iblk%mpiSize == mpiRank && 
				cur = logical_block[iblk].packed_molecules;
				while (cur != NULL) {
					molecule = (t_pack_molecule *) cur->data_vptr;
					if (molecule->valid) {
...
static t_pack_molecule *get_molecule_for_cluster(
    ...
    /****** MPI TODO: partitioning inside the function use %mpiSize == ********/
...
static t_pack_molecule* get_most_critical_seed_molecule(int * indexofcrit) {
	...
	while (*indexofcrit < num_logical_blocks) {
        ...
        /*********MPI TODO: Let process only choose the index looply added by comm_size **********/
        is_in_task = FALSE;
        if (is_run_global) {
            is_in_task = TRUE;
        } 
        if (blkidx%mpiSize == mpiRank) { 
            is_in_task = TRUE;
        } 
        if (!is_in_task) { 
            continue; 
...
static int my_MPI_Get_msg_size(int recv_rank, int recv_tag, MPI_Datatype recv_type) {
    MPI_Status status;
    int recv_size;
    MPI_Probe(recv_rank, recv_tag, MPI_COMM_WORLD, &status);
    MPI_Get_count(&status, recv_type, &recv_size);
    return recv_size;
}

static int arr2to1(int x, int y) {
    return (x*max_num_molecule + y);
}

static int arr3to1(int x, int y, int z) {
    return (x*max_num_molecule*max_molecule_num_blocks + y*max_molecule_num_blocks + z);
}
