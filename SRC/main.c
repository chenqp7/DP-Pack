...
int main(int argc, char **argv) {
	...
	entire_flow_end = clock();
		...
		vpr_printf(TIO_MESSAGE_INFO, "(main) #%d/%d << The entire flow of VPR took %g seconds.\n", mpiRank, mpiSize, (float)(entire_flow_end - entire_flow_begin) / CLOCKS_PER_SEC);
	#else
		vpr_printf(TIO_MESSAGE_INFO, "(main) #%d/%d << The entire flow of VPR took %g seconds.\n", mpiRank, mpiSize, (float)(entire_flow_end - entire_flow_begin) / CLK_PER_SEC);
	#endif
