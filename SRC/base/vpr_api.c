...
void vpr_pack(INP t_vpr_setup vpr_setup, INP t_arch arch) {
	...
	begin = clock();
	vpr_printf(TIO_MESSAGE_INFO, "(vpr_api) #%d/%d << Initialize packing.\n", mpiRank, mpiSize);
...
#define WIRE_SEGMENT_LENGTH 4
	...
	vpr_printf(TIO_MESSAGE_INFO, "(vpr_api) #%d/%d << Packing took %g seconds.\n", mpiRank, mpiSize,
			(float) (end - begin) / CLOCKS_PER_SEC);
	vpr_printf(TIO_MESSAGE_INFO, "(vpr_api) #%d/%d << Packing completed.\n", mpiRank, mpiSize);
#else
	vpr_printf(TIO_MESSAGE_INFO, "(vpr_api) #%d/%d << Packing took %g seconds.\n", mpiRank, mpiSize, (float)(end - begin) / CLK_PER_SEC);
#endif
	fflush(stdout);
}
